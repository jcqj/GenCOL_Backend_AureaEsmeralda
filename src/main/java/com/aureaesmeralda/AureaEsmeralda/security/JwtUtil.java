...
public class JwtUtil {
    ...
    // 1. Generar el Token al iniciar sesión con éxito
    public String generateToken(UserDetails userDetails) {
        Map<String, Object> claims = new HashMap<>();

        // Extraemos el rol del usuario de sus Authorities de Spring Security
        ...
        return Jwts.builder()
                ...
                .signWith(getSigningKey()) // Firmamos con la clave segura
                .compact();
    }

    // 2. Extraer el email (subject) guardado en el Token
    public String extractEmail(String token) {
        return extractClaims(token).getSubject();
    }

    // 3. Validar si el token coincide con el usuario y está vigente
    public boolean isTokenValid(String token, UserDetails userDetails) {
        final String email = extractEmail(token);
        return email.equals(userDetails.getUsername()) && !isTokenExpired(token);
    }

    // 4. Verificar si la fecha actual ya superó la fecha de expiración del token
    private boolean isTokenExpired(String token) {
        return extractClaims(token).getExpiration().before(new Date());
    }

    // 5. Parsea el contenido, valida la firma y extrae los Claims (Payload)
    private Claims extractClaims(String token) {
        return Jwts.parser()
                .verifyWith(getSigningKey()) // Asigna la clave de verificación de firma
                .build()
                .parseSignedClaims(token)
                .getPayload();
    }

    // 6. SOLUCIÓN: Convierte de forma segura el String Hexadecimal del properties a SecretKey
    private SecretKey getSigningKey() {
        // Decodifica la cadena Hexadecimal usando la herramienta nativa de Java
...
//! DOC PROFE ANA ORIGINAL, DESDE ACA.
//package com.aureaesmeralda.AureaEsmeralda.security;
//
//
//import io.jsonwebtoken.Claims;
